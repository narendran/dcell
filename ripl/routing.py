...
class StructuredRouting(Routing):
	...
	@param pkt parsed packet object, like one from POX
        ...
        for depth in range(lowest_starting_layer - 1, -1, -1):
            ...
            if paths_found:
                path_choice = self.path_choice(paths_found, src, dst, pkt)
...
class STStructuredRouting(StructuredRouting):
    ...
    def __init__(self, topo):
        ...
        def choose_leftmost(paths, src, dst, pkt):
            ...
            @param pkt parsed packet object, like one from POX (unused)
	    '''
            return paths[0]
...
#           print "  src_dpid = 0x%x" % src_dpid
#	    print "  dst_dpid = 0x%x" % dst_dpid
	    hash_input = [0] * 5
	    if isinstance(pkt.next, ipv4):
		ip = pkt.next
		hash_input[0] = ip.srcip.toUnsigned()
		hash_input[1] = ip.dstip.toUnsigned()
		hash_input[2] = ip.protocol
#		print "next: %s" % dir(pkt.next)
...
#	    print pkt.src # hex string
#	    print pkt.dst # hex string
#	    print pkt.type # string?
#	    # pkt.type == packet.IP_TYPE
#	    print dir(pkt.next)
#	    print "  path: %s" % choice
